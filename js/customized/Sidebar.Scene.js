...
function SidebarScene(editor) {
  ...
  /* AR Type Select Field */

  const arRow = new UIRow();
  const arType = new UISelect()
    .setOptions({
      None: "NONE",
  ...
  arRow.add(arType);
  container.add(arRow);
  editor.ui.arType = arType;

  /* HDR Rotation Angle */

  const rotationRow = new UIRow();
  const hdrRotation = new UINumber()
    .setStep(10)
    .setNudge(0.1)
    .setUnit("°")
    .setWidth("50px")
  ...
  editor.data.hdrRotation = hdrRotation.getValue();
  editor.ui.hdrRotation = hdrRotation;

  /* HDR Intensity Change */

  const hdrIntensityRow = new UIRow();
  const hdrIntensity = new UINumber(1.0)
    .setStep(0.1)
    .setNudge(0.1)
    .setUnit("")
    .setWidth("50px")
  ...
  editor.data.hdrIntensity = hdrIntensity.getValue();
  editor.ui.hdrIntensity = hdrIntensity;

  /* Model Path Input Field */

  const modelPathRow = new UIRow();
  const modelPathInput = new UIInput("")
    .setWidth("150px")
    .setFontSize("12px")
    .onChange(function () {
  ...
  editor.ui.modelPathInput = modelPathInput;

  /* HDR Path Input Field */

  const hdrPathRow = new UIRow();
  const hdrPathInput = new UIInput("")
    .setWidth("150px")
    .setFontSize("12px")
    .onChange(function () {
  ...
  editor.ui.hdrPathInput = hdrPathInput;
